...
def get_total_vols(main_file,FSLDIR):
    import os
    import subprocess   
...
def do_antspynet_bet(main_file,main_file_brain,main_file_brainmask,bet_params,bet_full_params,REALPATH):
    import ants
    import antspynet
    import os
    inImg = ants.image_read(main_file)
    brainSeg = antspynet.brain_extraction(inImg, modality=bet_params['modality'], antsxnet_cache_directory=os.path.join(REALPATH,'helper_functions','antsxnet_cache'), verbose=True)
...
# ******************* s3 bucket check ********************
def flirt_wf(IN_FILE,DATA_DIR,FLIRT_PARAMS,*args,**kwargs):
    """
    ...
    try:
        ...
        hf.get_dir_identifiers_new(mainFileDir)
        mainFlirtOutputDir = os.path.join(DATA_DIR,'derivatives','sub-' + hf.subject.id,'ses-' + hf.subject.fullSesNum,mainParams['output_bids_location'],workFlow.name)#create base path and filename for move
        mainBetOutputDir = os.path.join(DATA_DIR,'derivatives','sub-' + hf.subject.id,'ses-' + hf.subject.fullSesNum,mainParams['output_bids_location'],workFlow.name)#create base path and filename for move
        ...
        if refImage and not refImageParams['type'] == 'std':
            ref_regexStr = hf.get_bids_filename(**refImageParams['input_bids_labels'])
        ...
        mainBidsLabels = hf.get_bids_labels(mainFile)
        if secImage:
            ...
            sec_regexStr = hf.get_bids_filename(**secBidsLabels)
        ...
        if performFslRoi:
            ...
            if vols:
                ...
                outMainVolFile = os.path.join(mainBetOutputDir,hf.get_bids_filename(subject=hf.subject.id,session=hf.subject.fullSesNum,**roi_mainBidsLabels))
        ...
        if performBet:
            ...
            mainFile_brain = os.path.join(mainBetOutputDir,hf.get_bids_filename(subject=hf.subject.id,session=hf.subject.fullSesNum,**bet_mainBidsLabels))
            
            bet_mainBidsLabels['suffix'] = 'mask'
            mainFile_brainmask = os.path.join(mainBetOutputDir,hf.get_bids_filename(subject=hf.subject.id,session=hf.subject.fullSesNum,**bet_mainBidsLabels))
            ...
            if os.path.isfile(mainFile_brain) and not overwriteFlag:
                ...
                # -------------------------------
                if secImage:
                    secBidsLabels = hf.get_bids_labels(secFile)
                    ...
                    secFile_brain = os.path.join(mainBetOutputDir,hf.get_bids_filename(subject=hf.subject.id,session=hf.subject.fullSesNum,**secBidsLabels))
        ...
        if refImageFile:
            ...
            if refImageParams['type'] == 'std':
                ...
                if performFslRoi and 'brain' in mainFile:
                    ...
                    fltBidsLabels['description'] = 'brain'
            n_flt.inputs.out_file = os.path.join(mainFlirtOutputDir,hf.get_bids_filename(subject=hf.subject.id,session=hf.subject.fullSesNum,**fltBidsLabels))
            inputMatrixBase = hf.get_bids_labels(IN_FILE)
            ...
            if os.path.isfile(n_flt.inputs.out_matrix_file) and not overwriteFlag:
                if progress:
                ...
                if not os.path.isfile(n_invt.inputs.out_file) or overwriteFlag:
            ...
            if secImage:
                ...
                n_applyXfm.inputs.out_file = os.path.join(mainFlirtOutputDir,hf.get_bids_filename(subject=hf.subject.id,session=hf.subject.fullSesNum,**applyBidsLabels))
            ...
            if not refImageParams['type'] == "std" and stdImageFile:
                ...
                n_applyXfm2.inputs.out_file = os.path.join(mainFlirtOutputDir,hf.get_bids_filename(subject=hf.subject.id,session=hf.subject.fullSesNum,**applyBidsLabels))
                ...
                if secImage:
                    ...
                    for k in secImageParams['output_bids_labels'].keys():
                        applyBidsLabels[k] = secImageParams['output_bids_labels'][k]
                    n_applyXfm3.inputs.out_file = os.path.join(mainFlirtOutputDir,hf.get_bids_filename(subject=hf.subject.id,session=hf.subject.fullSesNum,**applyBidsLabels))
...
def main():
    ...
    flirt_wf(options.IN_FILE,options.DATA_DIR,options.FLIRT_PARAMS,**argsDict)
