...
# ******************* CREATE PYTHON JOB ********************
def create_python_condor_job(jobName,exeName,machineNames,submit,error,output,log,dagman):
    """
    ...
    job_out = Job(name=jobName, executable=os.path.join(REALPATH,'helper_functions',exeName), submit=submit, error=error, output=output, log=log, dag=dagman, getenv=True, extra_lines=extraLines, requirements=reqs, universe='vanilla')
